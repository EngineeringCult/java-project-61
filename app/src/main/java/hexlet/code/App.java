    ...
    private static void execute(Game selectedGame) {
        switch (selectedGame) {
            case GREET:
                ...
                break;
            case EVEN:
                Engine.run(Even.getMainQuestion(), Even.getExpressionResults());
                break;
            case CALC:
                Engine.run(Calc.getMainQuestion(), Calc.getExpressionResults());
                break;
            case GCD:
                Engine.run(Gcd.getMainQuestion(), Gcd.getExpressionResults());
                break;
            case PROGRESSION:
                Engine.run(Progression.getMainQuestion(), Progression.getExpressionResults());
                break;
            case PRIME:
                Engine.run(Prime.getMainQuestion(), Prime.getExpressionResults());
                break;
            default:
